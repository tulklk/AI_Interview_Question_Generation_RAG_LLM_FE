...
import { User, Globe, Moon, Bell, ShieldCheck, Shield, Check, ChevronRight, Zap, Star } from "lucide-react";
...
type Tab = "profile" | "language" | "appearance" | "notifications" | "security" | "privacy";
...
export function SettingsPage() {
  ...
  const tabs: { id: Tab; Icon: typeof User; iconBg: string; iconColor: string; label: string }[] = [
    { id: "profile",       Icon: User,       iconBg: "bg-violet-50 dark:bg-violet-950/40",  iconColor: "text-violet-500 dark:text-violet-400",  label: "Profile" },
    { id: "language",      Icon: Globe,      iconBg: "bg-blue-50 dark:bg-blue-950/40",      iconColor: "text-blue-500 dark:text-blue-400",      label: p.languageTitle },
    { id: "appearance",    Icon: Moon,       iconBg: "bg-indigo-50 dark:bg-indigo-950/40",  iconColor: "text-indigo-500 dark:text-indigo-400",  label: p.appearanceTitle },
    { id: "notifications", Icon: Bell,       iconBg: "bg-amber-50 dark:bg-amber-950/40",    iconColor: "text-amber-500 dark:text-amber-400",    label: p.notificationsTitle },
    { id: "security",      Icon: ShieldCheck, iconBg: "bg-emerald-50 dark:bg-emerald-950/40", iconColor: "text-emerald-500 dark:text-emerald-400", label: "Security" },
    { id: "privacy",       Icon: Shield,     iconBg: "bg-rose-50 dark:bg-rose-950/40",      iconColor: "text-rose-500 dark:text-rose-400",      label: p.privacyTitle },
  ];
  ...
  const sectionSubtitle: Record<Tab, string> = {
    ...
    language:      p.languageDescription,
    appearance:    p.appearanceDescription,
    notifications: p.notificationsDescription,
  ...
  return (
    <div className="flex flex-col md:grid md:grid-cols-[220px_1fr] gap-4 md:gap-6 items-start">
        ...
        <ul className="hidden md:flex md:flex-col gap-0.5">
          ...
          <li className="mt-3 pt-3 border-t border-gray-200 dark:border-gray-800">
            <div className="flex items-center gap-2.5 px-3 py-2">
              <div className="w-6 h-6 rounded-md flex items-center justify-center bg-amber-50 dark:bg-amber-950/40">
                <Star size={12} className="text-amber-500 dark:text-amber-400" />
              </div>
              <div className="flex-1 min-w-0">
                <p className={cn("text-xs font-semibold", portalHeading)}>Free Plan</p>
                <button
                  type="button"
                  className="text-[10px] font-semibold text-[#7C3AED] dark:text-[#a78bff] hover:underline"
                >
                  Upgrade →
                </button>
              </div>
      ...
      {/* ── Content panel ─────────────────────────────────────────────────────── */}
      {activeTab === "profile" ? (
        /* Profile: full layout, no extra wrapper padding */
        <div key="profile" className="animate-scale-in min-w-0 w-full">
          <CandidateProfile />
        </div>
      ) : (
      <div
