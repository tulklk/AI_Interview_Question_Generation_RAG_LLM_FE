...
import type { GenerationStatus } from "@/features/interview/types/generation-session";
...
export function SessionStatusBadge({ status, size = "sm" }: SessionStatusBadgeProps) {
  const { t } = useLanguage();
  const label =
    t.generationSessionPage.status[status as keyof typeof t.generationSessionPage.status] ??
    status;
