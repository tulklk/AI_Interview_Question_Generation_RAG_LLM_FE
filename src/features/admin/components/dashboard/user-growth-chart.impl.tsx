...
export default function UserGrowthChart() {
  ...
  return (
    <div
      ref={ref}
      className={cn(portalCard, "flex flex-col p-6 shadow-[0_4px_6px_-1px_rgba(0,0,0,0.1),0_2px_4px_-2px_rgba(0,0,0,0.1)] dark:shadow-none", isInView ? "animate-fade-up" : "opacity-0")}
    >
        ...
        {isInView ? (
          <ResponsiveContainer width="100%" height={220}>
            <AreaChart data={userGrowthData} margin={{ top: 4, right: 4, bottom: 0, left: -20 }}>
              ...
              <XAxis dataKey="week" tick={{ fontSize: 11, fill: chart.axisTickFill }} axisLine={false} tickLine={false} />
              ...
              />
              <Area type="monotone" dataKey="admins" name={ug.admins} stroke="#7c3aed" strokeWidth={2} fill="url(#gradAdmins)" dot={false} activeDot={{ r: 4, fill: "#7c3aed" }} />
              <Area type="monotone" dataKey="recruiters" name={ug.recruiters} stroke="#6c47ff" strokeWidth={2.5} fill="url(#gradRecruiters)" dot={false} activeDot={{ r: 4, fill: "#6c47ff" }} />
              <Area type="monotone" dataKey="guests" name={ug.guests} stroke="#a78bfa" strokeWidth={2.5} fill="url(#gradGuests)" dot={false} activeDot={{ r: 4, fill: "#a78bfa" }} />
            </AreaChart>
          </ResponsiveContainer>
        ) : (
